class PlayerTeamsController < ApplicationController
  ...
  def create
    ...
    if @playerteam.save
      ...
      redirect_back fallback_location: @team, flash: {notice: "Игрок добавлен в команду"}
    else
      redirect_back fallback_location: @team, flash: {"alert-danger": "Произошла ошибка: " + @playerteam.errors.full_messages.join(' ')}
    end
  end
  ...
  def destroy
    ...
    if @playerteam.destroy
      @userroletm.destroy
      redirect_back fallback_location: root_path, alert: "Игрок удален из команды"
    else
      redirect_back fallback_location: root_path, flash: {"alert-danger": "Произошла ошибка: " + @playerteam.errors.full_messages.join(' ')}
    end
  end
